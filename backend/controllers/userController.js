...
const updateUserProfile = async (req, res) => {
  try {
    ...
    // For sellers, allow business-related fields
    if (user.role === 'seller') {
      allowedFields.push(
        'businessName',
        'businessDescription',
...
const addAddress = async (req, res) => {
  try {
    ...
    const { title, firstName, lastName, phone, address, city, state, postalCode, country, isDefault } = req.body;
...
const getUserDashboard = async (req, res) => {
  try {
    ...
    // For sellers, get additional statistics
    let sellerStats = null;
    if (user.role === 'seller') {
      const productStats = await Product.aggregate([
        { $match: { seller: require('mongoose').Types.ObjectId(req.user.id) } },
        {
          $group: {
            _id: null,
            totalProducts: { $sum: 1 },
